...
class OperationalContentTests(unittest.TestCase):
    ...
    def test_links_resolve_to_resources(self):
        for data in (self.es,self.en):
            ...
            for r in data['resources'][-4:]:
                for rid in re.findall(r'href="#/recurso/(D\d{2})"',r['html']):
    ...
    def test_eight_browsing_policy_ids(self):
        for data in (self.es,self.en):
            self.assertEqual(set(re.findall(r'\bNAV-\d{2}\b',data['resources'][-2]['html'])),{f'NAV-{i:02}' for i in range(1,9)})
    def test_context_links_in_relevant_modules(self):
        for data in (self.es,self.en):
            for m in data['modules']:
                for rid in RELATED.get(m['id'],[]):
                    self.assertIn(f'href="#/recurso/{rid}"',m['html'])
    def test_missing_translation_fails_closed(self):
        def read(path):
            return '' if path.name=='operations-control.md' else build.read_text(path)
    ...
    def test_reordered_base_refused(self):
        with self.assertRaises(ValueError):
            add_resources([],build.COURSE,build.ROOT,build.REPO,build.markdown,build.read_text)
    ...
    def test_content_covers_non_equivalence_and_privacy(self):
        for data in (self.es,self.en):
            text=' '.join(r['html'] for r in data['resources'][-4:])
